...
if not os.path.exists(FEATURE_OUT_FILE):
    with open(FEATURE_OUT_FILE, 'w', newline='', encoding='utf-8') as f:
        f.write("timestamp,total_pkt_cnt,tcp_cnt,udp_cnt,icmp_cnt,syn_cnt,ack_cnt,unique_dst_port_cnt,unique_src_ip_cnt,dns_query_cnt,gratuitous_arp_cnt,failed_login_cnt,mac_change_cnt\n")
...
time_stats = defaultdict(lambda: {
    ...
    'syn_cnt': 0, 'ack_cnt': 0, 'unique_dst_port_cnt': 0, 
    'unique_src_ip_cnt': 0, 'dns_query_cnt': 0,       
    'gratuitous_arp_cnt': 0, 'failed_login_cnt': 0,    
    'mac_change_cnt': 0, 
    '_src_ips': set(), '_dst_ports': set() # 중복 제거를 위해 집합(set) 자료형 사용
})
...
def monitor_auth_log():
    try:
        with open(AUTH_LOG_PATH, 'r') as f:
            f.seek(0, 2) # 파일의 맨 끝(최신 로그)으로 이동 (리눅스의 'tail -f' 명령어와 같은 역할)
            while True:
                line = f.readline()
                if not line:
                    time.sleep(0.1) # 새 로그가 없으면 0.1초 쉬고 다시 확인 (CPU 100% 과부하 방지)
                    continue
...
while True:
    ...
    if current_bucket > last_processed_bucket:
        ...
        if last_processed_bucket in time_stats:
            ...
            row = [
                last_processed_bucket, s['total_pkt_cnt'], s['tcp_cnt'], s['udp_cnt'], 
                s['icmp_cnt'], s['syn_cnt'], s['ack_cnt'], len(s['_dst_ports']), 
                len(s['_src_ips']), s['dns_query_cnt'], s['gratuitous_arp_cnt'], 
                s['failed_login_cnt'], s['mac_change_cnt']
            ]
